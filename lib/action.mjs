...
function uploadAll(ssh, source, remote) {
  function sendFile(file) {
    return ssh.putFile(file.local, file.remote).then(
      () => {
  ...
  return scan(source, remote).then((groups) =>
    withTimeout(() => pSeries(groups.map((group) => () => sendGroup(group)))),
  );
}
